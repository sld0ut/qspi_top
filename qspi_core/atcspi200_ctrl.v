...
input	[31:0]		i_spi_setcount; //bruce
`ifdef ATCSPI200_MEM_SUPPORT
input			arb_mem_req;
...
assign arb_wr_num	= i_spi_setcount[31:16]; //bruce
assign arb_dummy_num	= arb_trans_ctrl[10:9];
//assign arb_rd_num	= arb_trans_ctrl[8:0];
assign arb_rd_num	= i_spi_setcount[15:0]; //bruce
...
always @(*)
begin
	if ((~spi_master & spi_cs_deassert) | (arb_mem_req & ~ctrl_req))
...
assign spi_trans_w_end    = arb_mem_req ? ~ctrl_req : (spi_txdata_rd & tx_unit_hit & (data_cnt_r == arb_wr_num));
assign spi_trans_r_end    = arb_mem_req ? ~ctrl_req : (spi_txdata_rd & tx_unit_hit & (data_cnt_r == arb_rd_num));
...
assign data_cnt_clr = tx_bit_cnt_clr | ((((ctrl_cs_r == SPI_CTRL_W_ONLY) & (data_cnt_r == arb_wr_num)) | ((ctrl_cs_r == SPI_CTRL_R_ONLY) & (data_cnt_r == arb_rd_num))) & arb_mem_req & spi_txdata_rd & tx_unit_hit);
	...
	if (tx_byte_hit) begin
		if (arb_data_merge) begin
			if ((ctrl_cs_r == SPI_CTRL_CMD) | (ctrl_cs_r == SPI_CTRL_TOKEN) | (ctrl_cs_r == SPI_CTRL_DUMMY) | (ctrl_cs_r == SPI_CTRL_S_DUMMY))
				...
				tx_word_hit = (tx_bit_cnt_r[4:3] == 2'h3) | ((ctrl_cs_r == SPI_CTRL_W_ONLY) & arb_mem_req & (data_cnt_r == arb_wr_num));
		end
		else
...
begin
	...
	else if (spi_reset_sclk | (arb_mem_req & ~ctrl_req))
		rx_shift_reg_full_r <= 1'b0;
