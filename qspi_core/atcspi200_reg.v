...
output	[31:0]		o_spi_setcount; //bruce
...
wire			reg_txf_wr;
wire			reg_rxf_rd;
wire [`ATCSPI200_TXFPTR_BITS-1:0]	txf_threshold;
wire [`ATCSPI200_RXFPTR_BITS-1:0]	rxf_threshold;

wire	[19:0]		atcspi200_id;
wire	[7:0]		atcspi200_rev_major;
wire	[3:0]		atcspi200_rev_minor;
wire			slave_sup;
wire			reg_pio_sup;
wire			quadspi_sup;
wire			dualspi_sup;
wire			eilm_mem_sup;
wire			ahb_mem_sup;
wire	[1:0]		txf_depth_inf;
wire	[1:0]		rxf_depth_inf;
wire			reg_txf_empty;
wire			reg_rxf_full;
`ifdef ATCSPI200_SLAVE_SUPPORT
reg			slv_cmd_int_r;
...
reg			reg_req_r;
reg	[31:0]		reg_spi_setcount_r; //bruce
...
`ifdef ATCSPI200_SLAVE_SUPPORT
	...
	//reg	[8:0]	slave_rcnt_r;
	//reg	[8:0]	slave_wcnt_r;
	reg	[15:0]	slave_rcnt_r; //bruce
	reg	[15:0]	slave_wcnt_r; //bruce
`else
	wire		spi_master;
`endif
...
`ifdef ATCSPI200_MEM_SUPPORT
	wire		reg_mem_spi_wr;
`endif
wire			reg_spi_setcount_wr; //bruce
`ifdef ATCSPI200_SLAVE_SUPPORT
...
`endif
assign reg_spi_setcount_wr      = reg_wr_a & (reg_waddr==5'h15); //bruce
`ifdef ATCSPI200_SLAVE_SUPPORT
...
begin
	if (~regrstn)
		reg_req_r <= 1'b0;
	else if (reg_trans_end_regclk | spi_reset_regclk)
		reg_req_r <= 1'b0;
...
assign reg_req_regclk = reg_req_r;
...
begin
	if (~regrstn)
		reg_reg_ctrl_r[14:3] <= 12'h0;
	else if (reg_reg_ctrl_wr)
		reg_reg_ctrl_r[14:3] <= {reg_wdata[20:16], reg_wdata[12:8], reg_wdata[4:3]};
end

assign txf_threshold = reg_reg_ctrl_r[10 +: `ATCSPI200_TXFPTR_BITS];
assign rxf_threshold = reg_reg_ctrl_r[ 5 +: `ATCSPI200_RXFPTR_BITS];
...
begin
	if (~regrstn)
		...
		txf_thres_int_r <= 1'b0;
	else if (reg_int_en[3] & (reg_txf_entries <= txf_threshold))
...
begin
	if (~regrstn)
		...
		rxf_thres_int_r <= 1'b0;
	else if (reg_int_en[2] & (reg_rxf_entries >= rxf_threshold))
...
begin
	if (~regrstn)
		reg_spi_setcount_r <= 32'b0;
	else if (reg_spi_setcount_wr)
		reg_spi_setcount_r <= reg_wdata[31:0];
end
...
/*
	...
	begin
		if (~regrstn)
			...
			slave_rcnt_r <= slave_rcnt_r + 9'h1;
	end
	...
	begin
		if (~regrstn)
			...
			slave_wcnt_r <= slave_wcnt_r + 9'h1;
	end
*/
	...
	begin
		if (~regrstn)
			...
			slave_rcnt_r <= slave_rcnt_r + 16'h1;
	end
	...
	begin
		if (~regrstn)
			...
			slave_wcnt_r <= slave_wcnt_r + 16'h1;
	end

`endif

assign atcspi200_id        = `ATCSPI200_ID;
assign atcspi200_rev_major = `ATCSPI200_REV_MAJOR;
assign atcspi200_rev_minor = `ATCSPI200_REV_MINOR;
...
begin
	if (~regrstn)
		...
		case(reg_raddr)
			5'h00: reg_rdata_r <= {atcspi200_id, atcspi200_rev_major, atcspi200_rev_minor};
			...
			5'h0c: reg_rdata_r <= {11'h0, reg_reg_ctrl_r[14:10], 3'h0, reg_reg_ctrl_r[9:5], 3'h0, reg_reg_ctrl_r[4:3], txf_clr_level, rxf_clr_level, spi_reset_sysclk};
		`ifdef	ATCSPI200_TXFIFO_DEPTH_16W
			5'h0d: reg_rdata_r <= {8'h0, reg_txf_full, reg_txf_empty, 1'b0, reg_txf_entries, reg_rxf_full, reg_rxf_empty, 1'h0, reg_rxf_entries, 7'h0, reg_busy_status};
		`else
			5'h0d: reg_rdata_r <= {8'h0, reg_txf_full, reg_txf_empty, 1'b0, {{(5-`ATCSPI200_TXFPTR_BITS){1'b0}},reg_txf_entries}, reg_rxf_full, reg_rxf_empty, 1'h0, {{(5-`ATCSPI200_RXFPTR_BITS){1'b0}}, reg_rxf_entries}, 7'h0, reg_busy_status};
		`endif
			...
			`endif
			5'h15: reg_rdata_r <= reg_spi_setcount_r; //bruce
			`ifdef ATCSPI200_SLAVE_SUPPORT
				5'h18: reg_rdata_r <= {13'h0, slave_status_r};
			//	5'h19: reg_rdata_r <= {7'h0, slave_wcnt_r, 7'h0 ,slave_rcnt_r};
				5'h19: reg_rdata_r <= {slave_wcnt_r[15:0], slave_rcnt_r[15:0]}; //bruce
			`endif
			5'h1f: reg_rdata_r <= {17'h0, slave_sup, eilm_mem_sup, ahb_mem_sup, reg_pio_sup, 1'b0, quadspi_sup, dualspi_sup, 2'h0, txf_depth_inf, 2'h0, rxf_depth_inf};
			default: reg_rdata_r <= 32'h0000;
		endcase
	end
end

assign reg_busy_status		= reg_req_r | reg_busy;
...
assign o_spi_setcount		= reg_spi_setcount_r ; //bruce
endmodule
