...
`ifdef ATCSPI200_SLAVE_SUPPORT
	assign spi_busy = spi_master ? (spi_cs_r != SPI_M_IDEL) : ~spi_cs_n_in_syn;
`else
	assign spi_busy = spi_cs_r != SPI_M_IDEL;
`endif
...
CLK_MUX u_spi_clock_inv (.A(~spi_clock), .B(spi_clock), .S(scan_test), .Y(spi_clock_inv));
...
gck_sys master_gclk_0 (.clk_out(master_gclk_org), .clk_en(master_clk_en),     .clk_in(spi_clock), .test_en(scan_enable));
//gck_sys master_gclk_1 (.clk_out(master_gclk_d),   .clk_en(master_clk_d_en_r), .clk_in(~spi_clock), .test_en(scan_enable));
gck_sys master_gclk_1 (.clk_out(master_gclk_d),   .clk_en(master_clk_d_en_r), .clk_in(spi_clock_inv), .test_en(scan_enable));

`ifdef	FPGA
assign master_gclk = 1'b0;
`else
...
begin
	if (!spi_rstn)
		master_cs_r <= 1'b1;
	else if (!spi_master | spi_reset_sclk | (spi_ns == SPI_M_IDEL) | (spi_ns == SPI_M_CS))
		master_cs_r <= 1'b1;
	else
		master_cs_r <= 1'b0;
end
...
begin
	...
	else if (!spi_master | spi_reset_sclk | (spi_cs_r == SPI_M_IDEL))
		master_rxdata_wr_lvl_r <= 1'b0;
...
`ifdef ATCSPI200_SLAVE_SUPPORT
	...
	defparam spi_cs_n_syn.RESET_VALUE = 1'b1;
	nds_sync_l2l spi_cs_n_syn (
		.b_reset_n(spi_rstn),
		.b_clk(spi_clock),
	...
	assign spi_cs_deassert  = spi_master ? ((spi_cs_r == SPI_M_CS) & (spi_ns   == SPI_M_IDEL)) : spi_cs_in_posedge;
...
`else
	...
	assign spi_cs_deassert  = (spi_cs_r == SPI_M_CS) & (spi_ns == SPI_M_IDEL);
`endif
...
`ifdef ATCSPI200_SLAVE_SUPPORT
	...
	begin
		if (!spi_rstn)
			spi_oe_slv_r <= {SPI_OE_WIDTH{1'b1}};
		else if (~spi_master) begin
			`ifdef ATCSPI200_QUADSPI_SUPPORT
...
`ifdef ATCSPI200_DIRECT_IO_SUPPORT
	...
	assign spi_miso_out	= pio_enable ? pio_miso_out : spi_master ? spi_out_r[1] : spi_out_slv_r[1];
...
`else
	...
	assign spi_miso_out	= spi_master ? spi_out_r[1] : spi_out_slv_r[1];
...
always @(negedge spi_rstn or posedge spi_r_clk)
begin
	if (~spi_rstn) begin
`ifdef ATCSPI200_QUADSPI_SUPPORT
	spi_in_r[3] <= 'd0;
